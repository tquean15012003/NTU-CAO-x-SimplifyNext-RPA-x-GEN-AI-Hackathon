...
NLP_CONFIGURATION = [{"lang_code": "en", "model_name": "en_core_web_md"}]


SUPPORTED_LANG_CODES = [item["lang_code"] for item in NLP_CONFIGURATION]


def load_base_entities_and_masking_function(
...
def load_custom_regex_and_masking_function(
    seed: Optional[int] = None,
) -> List[Dict[str, Any]]:
    fake = Faker()
    fake.seed_instance(seed)
    return [
        ...
        {
            "name": "NTU_MARTRIC_NUMBER",
            "regex_list": ["^[UG][A-Za-z0-9]{7}[A-Z]$"],
            "context_list": ["matric", "number"],
            "masking_function": lambda _: fake.bothify(text="????####?").upper(),
        },
    ]
